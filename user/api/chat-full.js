...
async function handleArticleQuery(articleNumber, message) {
  try {
    ...
    const documents = await Document.find({ status: 'ready' }).select('title category content');
...
// Tìm kiếm tài liệu đơn giản (fallback khi không có vector search)
async function searchDocuments(query, limit = 5) {
  try {
    ...
    }).limit(limit).select('title category content');
